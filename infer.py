...
from src.utils import load_data_for_inference, load_data_for_inference_xm3600, prep_strings, postprocess_preds, translate_caption
...
def evaluate_norag_model(args, feature_extractor, tokenizer, model, eval_df):
    ...
    template = open(args.template_path).read().split("||")[1].strip() + ' '
    ...
    if args.features_path is not None:
        ...
        for idx in tqdm(range(len(eval_df))):
            ...
            decoder_input_ids = prep_strings('', tokenizer, template=template, is_test=True) 
            ...
            pred = postprocess_preds(pred, tokenizer).strip()
    ...
    # load image 
        ...
        for idx in tqdm(range(0, len(eval_df), bs)):
                ...
                decoder_input_ids = [prep_strings('', tokenizer, template=template, is_test=True) for _ in range(len(image_ids))] 
...
def evaluate_rag_model(args, feature_extractor, tokenizer, model, eval_df):
    ...
    for idx in tqdm(range(len(eval_df))):
        ...
        #     caps=[translate_caption(translation_model, translation_tokenizer, cap, target_language) for cap in caps]
...
def infer_one_checkpoint(args, feature_extractor, tokenizer, checkpoint_path, eval_df, infer_fn, language="en"):
    ...
    with open(os.path.join(checkpoint_path, args.outfile_name), 'w') as outfile:
        json.dump(preds, outfile, ensure_ascii=False)
...
def register_model_and_config():
    ...
    AutoConfig.register("paella", SmallCapConfig)
    AutoModel.register(SmallCapConfig, SmallCap)
...
def main(args):
    ...
    if args.disable_rag:
        ...
        if args.caps_in_encoder:
            infer_fn = evaluate_enc_rag_model
        else:
    ...
    data = load_data_for_inference(args.annotations_path,split, args.captions_path)

    eval_df = pd.DataFrame(data[split])
    template_lg=args.template_path.split("_")
    if len(template_lg)>1:
        language=template_lg[-1]
    else:
        language="en"
    
    args.outfile_name = '{}_{}_{}_preds.json'.format(language, split, args.dataset)

    print(" args.outfile_name", args.outfile_name)
    ...
    args.generation_kwargs = {'max_new_tokens': CAPTION_LENGTH, 'no_repeat_ngram_size': args.no_repeat_ngram_size, 'length_penalty': 0.,
                              'num_beams': 3, 'early_stopping': True, 'eos_token_id': tokenizer.eos_token_id,  'min_new_tokens':args.min_new_tokens
    }
    ...
    if args.checkpoint_path is not None:
        ...
        infer_one_checkpoint(args, feature_extractor, tokenizer, checkpoint_path, eval_df, infer_fn, args.language)
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--batch_size", type=int, default=32, help="Batch size; only matter if evaluating a norag model")
