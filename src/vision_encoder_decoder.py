...
class SmallCap(PreTrainedModel):
    ...
    def from_encoder_decoder_pretrained(
        ...
        **kwargs
        ...
        if decoder is None:
            ...
            if "config" not in kwargs_decoder:
                if "xglm" in decoder_pretrained_model_name_or_path:
                    # if "2.9" in decoder_pretrained_model_name_or_path:
                    #     projection=1024
                    # elif "1.7" in decoder_pretrained_model_name_or_path:
                    #     projection=1024
                ...
                #decoder_config.enc_text_dim = text_dim
                #decoder_config.enc_vision_dim = encoder.config.vision_config.hidden_size

                print("encoder.config.vision_config.hidden_size",encoder.config.vision_config.hidden_size)

                # if projection:
                #     decoder_config.projection= projection
        ...
        print("I add text proje")

        #new_module=nn.Linear(text_dim, encoder.config.vision_config.hidden_size, bias=False)
        #setattr(encoder, "my_text_projection", new_module)

        #encoder.layers=new_module
        #encoder._modules.my_text_projection = nn.Linear(text_dim, encoder.config.vision_config.hidden_size, bias=False)
        print("vars", vars(decoder))
        # make sure input & output embeddings is not tied
        config.tie_word_embeddings = False
